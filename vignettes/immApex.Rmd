...
# Keras3 check function
check_keras3 <- function() {
  if (!requireNamespace("keras3", quietly = TRUE)) return(FALSE)
  if (!reticulate::py_module_available("keras")) return(FALSE)
  
  # Try importing keras.ops to catch runtime errors
  tryCatch({
    reticulate::import("keras.ops", delay_load = FALSE)
    TRUE
  }, error = function(e) {
    message("Skipping keras3-dependent code due to missing keras.ops.")
    FALSE
  })
}

use_keras3 <- check_keras3()

# Disable evaluation of any code chunk if keras3 isn't usable
knitr::opts_chunk$set(eval = use_keras3)
```
